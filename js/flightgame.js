...
function capsul (type, power) {
    var obj = {};
    obj.type = type;
    obj.image = new Image();
    obj.image.src = "../img/FlightGame/capsulsprites.png";
    obj.vx = Math.random() * 2;
    obj.hp = 1;
    obj.powerup = power;
    obj.x = 50;
    ...
    obj.draw = function () {
        if(obj.os){
            context.drawImage(  obj.image, 
                (Math.floor(obj.index) * (obj.fw/obj.frames)), 
                0, 
                (obj.fw/obj.frames), 
                obj.h, 
                obj.x, 
...
function powerup (x, y) {
    var obj = {};
    obj.image = new Image();
    obj.image.src = "../img/FlightGame/powerup.png";
    obj.x = x;
    obj.y = y; 
    obj.vy = 5;
    obj.h = 25;
    ...
    function makeships () {
        ...
        if(game_level % 2){
            power_capsul = new capsul();
            capsul_array.push(power_capsul);
        }
    }
    ...
    function draw() {
        ...
        if (LoseCondition){         
            ...
            lose.image.src = "../img/FlightGame/warpdrive.png";    // font in image is 'i_fink_u_freeky' by author Frédéric Rich
            ...
            context.drawImage(lose.image, 100, canvas.height/6);
            context.font = "30px Anton, sans-serif";
            context.fillStyle = "white";
            context.fillText("PRESS 'R' TO START",canvas.width*.35, canvas.height*.75);
        }
        else{
            // Draw my player
            ...
            // Draw capsuls
            for (var i = 0; i < capsul_array.length; i++) {
    ...
    function update(LoseCondition) {
        if(LoseCondition){
            ...
            for (var i = 0; i < capsul_array.length; i++) {
                capsul_array[i].update();
                if(!capsul_array[i].os){
                    var x_drop = capsul_array[i].x;
                    var y_drop = capsul_array[i].y;
                    var drop = new powerup(x_drop, y_drop);
                    capsul_array.splice(i,1);
                    var boom = new explosion(x_drop, y_drop);
                    explosion_array.push(boom);
                    powerup_array.push(drop);
                    }
            }
            for (var i=0; i < bullet_array.length; i++) {
